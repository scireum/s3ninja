...
class SignedChunkHandler extends sirius.web.http.InputStreamHandler {
    ...
    private int transferNextChunk() throws IOException {
        ...
        if (length == 0 && contentSHA256Content.contains(contentSHA256Header)) {
            ...
            AtomicBoolean isChecksumHeader = new AtomicBoolean(false);
            List<String> checksumHeaders = List.of("x-amz-checksum", "x-amz-sdk-checksum-algorithm");
            isChecksumHeader.set(checksumHeaders.stream().anyMatch(checksum::contains));
            if (isChecksumHeader.get() && signature.contains("x-amz-trailer")) {
                chunkBuffer.readerIndex(chunkBuffer.readerIndex() + 2);
